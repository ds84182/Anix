...
function term.sendEvent(event)
	...
	srv:invoke("sendEvent", handle, event)
end
...
function term.getState()
	...
	term.sendEvent {type = "get_state", done = completer}
...
function term.getSize()
	...
	term.sendEvent {type = "get_resolution", done = completer}
	
	local size = await(future)
...
function term.setCursor(x, y)
	...
	term.sendEvent {type = "move_cursor", x = x, y = y}
end
...
function term.setCursorBlink(blink)
	...
	term.sendEvent {type = "set_blink", blink = blink}
end
...
function term.write(str)
	...
	term.sendEvent {type = "write", text = str}
end

function term.put(x, y, str)
	str = tostring(str)
	
	term.sendEvent {type = "put", x = x, y = y, text = str}
end
...
function term.copy(x, y, width, height, destx, desty)
	...
	term.sendEvent {type = "copy", x = x, y = y, width = width, height = height, destx = destx, desty = desty}
end
